...
class EM_learner():
    ...
    def em_step(self, orig_class_counts, orig_feature_counts):
        ...
        feature_counts = [[0] * self.num_classes for feat in range(self.dataset.shape[1])]

        for tple in self.dataset:
            ...
            for cl in range(self.num_classes):
                ...
                for (ind,feat) in enumerate(range(self.dataset.shape[1])):
                    feature_counts[ind][feat(tple)][cl] = \
                        self.feature_counts[ind][feat(tple)][cl] + \
                        tpl_class_dist[cl]
